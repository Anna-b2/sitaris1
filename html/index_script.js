...
function createPage() {	
	...
	data["visit"] = document.getElementsByName("visit")[0].value;
    ...
    var html = '<html>';
    html+='<head><title>Результат опроса</title><link rel="stylesheet" href="../css/style.css"></head><body>';
    ...
    html+="<tr><td>№</td><td>Имя</td><td>Возраст</td><td>Ассортимент для расширения</td><td>Посещение</td><td>Удовлетворенность</td>" +
        "<td>Цвет</td>" +
	"<td>Предложения</td></tr>";
 
	for (var i=0; i < count; i++) {
        var j = i + 1;
        html += "<tr><td>" + j + "</td><td>" + all_data[i].name + "</td><td>" + all_data[i].age + 
        "</td><td>" + all_data[i].range + "</td><td>" + all_data[i].visit + "</td>" +
